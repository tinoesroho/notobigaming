...
#define PLUGIN_NAME "Round Songs (NotObi Fork)"
#define PLUGIN_AUTHOR "1Swat2KillThemAll + NotObigaming Team"
...
public Action:Command_say(client, args) {
	...
	if(strcmp(text[startidx], "sm_music", false)==0 || strcmp(text[startidx], "/music", false)==0)  {
	g_bIsListening[client] = !g_bIsListening[client];
	...
	if(strcmp(text[startidx], "radio", false)==0 || strcmp(text[startidx], "!radio", false)==0)  {
	g_bIsListening[client] = !g_bIsListening[client];
	...
	if(strcmp(text[startidx], "/sound", false)==0 || strcmp(text[startidx], "!sound", false)==0)  {
	g_bIsListening[client] = !g_bIsListening[client];
...
StartSound(bool:EndFirst = true)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			EmitSoundToClient(i, buff, SOUND_FROM_PLAYER, SNDCHAN_AUTO, g_SndLevel[i], SND_NOFLAGS, ((g_SndVolume[i])*0.25));
		}
	}

	GetArrayString(g_h_arr_strNameList, index, buff, sizeof(buff));
	PrintToChatAll("[SM] Now Playing: %s", buff);
