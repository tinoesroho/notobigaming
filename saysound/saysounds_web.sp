...
public Plugin: myinfo = {
    name = "Saysounds SLLE",
    author = "notObigaming",
    description = "Saysounds(Slightly Less Lite Edition)",
    version = "1.4",
    url = ""
};
...
public Action: OnStop(int client, int args) {
    OpenPanel("Hidden Panel", "about:blank", client);
    PrintToChat(client, "[notIGA] Playback stopped.");
...
public Action: Command_Say(client,
...
static Sound_Play(client,
    const String: speech[], channel) {
    decl String: filelocation[PLATFORM_MAX_PATH + 1];
    decl String: filelocationFake[PLATFORM_MAX_PATH + 1];
            ...
            if (filelocation[0] != '\0') {
                if (g_lastplay[client] < thetime) {
                    if (speech[0] && IsValidClient(client)) {
                        ...
                        Format(filelocationFake, sizeof(filelocationFake), "*%s", filelocation);
                        StrCat(realurl, sizeof(realurl), filelocationFake);
                        ReplaceString(realurl, sizeof(realurl), "*", "");
                        ReplaceString(realurl, sizeof(realurl), ".mp3", "");
                        for (int i = 1; i <= MaxClients; i++) {
                            if (IsClientInGame(i) && (!IsFakeClient(i))) {
                                if (ClientHasSaysoundEnabled(i)) {
                                    OpenPanel("Hidden Panel", realurl, i);
                                }
                            }
                        }
                    }
