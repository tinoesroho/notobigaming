...
{
	name = "NotObigaming Ranking",
	author = "Lash + Dionysus + Mike + R_Hehl",
	description = "Ranks players according to skill.",
	version = PL_VERSION,
	url = "http://steamcommunity.com/groups/notobigaming"
};
public OnPluginStart() {
	...
	if(sqlite) {
		SQL_TQuery(db, SQLErrorCheckCallback, "CREATE TABLE IF NOT EXISTS stattest (steamid TEXT, name TEXT, rating INTEGER, kills INTEGER, deaths INTEGER, notify INTEGER)");
	} else {
		SQL_TQuery(db, SQLErrorCheckCallback, "CREATE TABLE IF NOT EXISTS stattest (steamid varchar(32) NOT NULL, name varchar(64) NOT NULL, rating int(8) NOT NULL, kills int(8) NOT NULL, deaths int(8) NOT NULL, notify int(2) NOT NULL)");
	}
	CreateConVar("sm_nobi_version", PL_VERSION, "NotObigaming Ranking version.", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public OnClientPutInServer(client) {
	if(!IsFakeClient(client)) {
		...
		Format(query, sizeof(query), "SELECT rating,kills,deaths,notify FROM stattest WHERE steamid='%s'", clientid);
		SQL_TQuery(db, SQLQueryConnect, query, userid);
		sessionrating[client] = 0;
		sessionkills[client] = 0;
		sessiondeaths[client] = 0;
		SQL_TQuery(db, SQLQueryTop10, "SELECT name,rating FROM stattest ORDER BY rating DESC LIMIT 0,10", userid);
	}
}
public SQLQueryConnect(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if(hndl==INVALID_HANDLE) {
		...
		if(!SQL_MoreRows(hndl)) {
			if(sqlite) {
				Format(query, sizeof(query), "INSERT INTO stattest VALUES('%s', '%N', 1600, 0, 0, 1)", clientid, client);
				SQL_TQuery(db, SQLErrorCheckCallback, query);
			} else {
				Format(query, sizeof(query), "INSERT INTO stattest (steamid, name, rating, kills, deaths, notify) VALUES ('%s', '%N', 1600, 0, 0, 1)", clientid, client);
				SQL_TQuery(db, SQLErrorCheckCallback, query);
			}
			rating[client] = 1600;
			kills[client] = 0;
			...
			Format(query, sizeof(query), "UPDATE stattest SET name='%s' WHERE steamid='%s'", client, clientname, clientid);
			SQL_TQuery(db, SQLErrorCheckCallback, query);
		}
...
public SQLQueryRank(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	SQL_TQuery(db, SQLQueryCount, "SELECT COUNT(*) FROM stattest", GetClientUserId(client));
}
	...
	if(!roundend) {
		...
		if(client!=0 && attacker!=0) {
			if(!IsFakeClient(client)&&!IsFakeClient(attacker)&&client!=attacker) {
				...
				// <terribly_broken> - LASH
				// Insert score multipliers here
				decl String:weapon[64];
				GetEventString(event, "weapon", weapon, sizeof(weapon));
				ReplaceString(weapon,sizeof(weapon),"weapon_","");
	
				new  Float:score_bonus = 1.5;
								
					// Melee
				if(StrEqual(weapon,"physics")){
					score_bonus  = 1.5;
				}	
				if(StrEqual(weapon,"axe")){
					score_bonus  = 0.8;
				}
				if(StrEqual(weapon,"thrown_axe")){
					score_bonus  = 0.9;
				}
				...
				if(StrEqual(weapon,"thrown_knife")){
					score_bonus  = 1.0;
				}
				if(StrEqual(weapon,"fists")){
					score_bonus  = 1.5;
				}
				if(StrEqual(weapon,"fists_brass")){
					score_bonus  = 1.3;
				}
				if(StrEqual(weapon,"bow")){
					score_bonus  = 1.1;
				}
				if(StrEqual(weapon,"machete")){
					score_bonus  = 0.6;
				}
				if(StrEqual(weapon,"thrown_machete")){
					score_bonus  = 0.7;
				}
				// Handguns
				if(StrEqual(weapon,"thrown_gun")){
					score_bonus  = 1.2;
				}
				if(StrEqual(weapon,"coltnavy")){
					score_bonus  = 1.0;
				}
				if(StrEqual(weapon,"coltnavy2")){
					score_bonus  = 1.0;
				}
				if(StrEqual(weapon,"deringer")){
					score_bonus  = 1.0;
				}
				if(StrEqual(weapon,"deringer2")){
					score_bonus  = 1.0;
				}
				if(StrEqual(weapon,"volcanic")){
					score_bonus  = 1.0;
				}
				if(StrEqual(weapon,"volcanic2")){
					score_bonus  = 1.0;
				}
				if(StrEqual(weapon,"schofield")){
					score_bonus  = 0.7;
				}
				if(StrEqual(weapon,"schofield2")){
					score_bonus  = 0.7;
				}
				if(StrEqual(weapon,"maresleg")){
					score_bonus  = 0.6;
				}
				if(StrEqual(weapon,"maresleg2")){
					score_bonus  = 0.6;
				}
				if(StrEqual(weapon,"remington_army")){
					score_bonus  = 0.7;
				}
				if(StrEqual(weapon,"remington_army2")){
					score_bonus  = 0.7;
				}
				if(StrEqual(weapon,"sawedoff_shotgun")){
					score_bonus  = 0.6;
				}
				if(StrEqual(weapon,"sawedoff_shotgun2")){
					score_bonus  = 0.6;
				}
				if(StrEqual(weapon,"walker")){
					score_bonus  = 0.4;
				}
				if(StrEqual(weapon,"walker2")){
					score_bonus  = 0.4;
				}
				// Shotguns
				if(StrEqual(weapon,"coachgun")){
					score_bonus  = 0.6;
				}
				if(StrEqual(weapon,"shotgun")){
					...
					// Explosives
				if(StrEqual(weapon,"dynamite")){
					score_bonus  = 1.2;
				}
				if(StrEqual(weapon,"dynamite_black")){
					score_bonus  = 1.2;
				}
					// Rifles
				if(StrEqual(weapon,"henryrifle")){
					score_bonus  = 0.7;
				}
				if(StrEqual(weapon,"spencer")){
					score_bonus  = 0.6;
				}
				if(StrEqual(weapon,"sharps")){
					score_bonus  = 0.4;
				}
				if(StrEqual(weapon,"carbine")){
					score_bonus  = 0.8;
				}
				...
				new prebonus_diff = RoundFloat(GetConVarFloat(g_kValue)*(1-prob));
				new diff = RoundFloat(prebonus_diff * score_bonus);
				// end score multipliers
				// </terribly_broken>
				...
				Format(query, sizeof(query), "UPDATE stattest SET rating=%i,deaths=%i WHERE steamid='%s'", rating[client], deaths[client], clientid);
				SQL_TQuery(db, SQLErrorCheckCallback, query);
				Format(query, sizeof(query), "UPDATE stattest SET rating=%i,kills=%i WHERE steamid='%s'", rating[attacker], kills[attacker], attackerid);
				SQL_TQuery(db, SQLErrorCheckCallback, query);
				if(notify[client]) {
					if(colour) {
...
public Action:Command_say(client, args) {
	...
	if(strcmp(text[startidx], "/elo_notify", false)==0 || strcmp(text[startidx], "elo_notify", false)==0) {
		...
		if(notify[client]==false) {
			...
			Format(query, sizeof(query), "UPDATE stattest SET notify=1 WHERE steamid='%s'", clientid);
			SQL_TQuery(db, SQLErrorCheckCallback, query);
		} else {
			notify[client] = false;
			if(colour) {
			...
			Format(query, sizeof(query), "UPDATE stattest SET notify=0 WHERE steamid='%s'", clientid);
			SQL_TQuery(db, SQLErrorCheckCallback, query);
		}
		return Plugin_Handled;
		...
		if(StrContains(text[startidx], "rank", false)!=-1) {
			...
			Format(query, sizeof(query), "SELECT COUNT(*) FROM stattest WHERE rating>%i", rating[client]);
		...
		SQL_TQuery(db, SQLQueryTop10, "SELECT name,rating FROM stattest ORDER BY rating DESC LIMIT 0,10", GetClientUserId(client));
		return Plugin_Handled;
	}
	return Plugin_Continue;	
}
